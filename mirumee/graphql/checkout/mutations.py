...
class OrderComplete(graphene.Mutation):
    ...
    def validate_input(cls, input, checkout_id):
        ...
        def validate_checkout(checkout_id):
            if not Checkout.objects.filter(id=checkout_id).exists():
                raise ValidationError("This checkout (ID: {checkout_id}) does not exist.")
            
        input['']'''

    @classmethod
    def mutate(cls, root, _info, input, checkout_id, user_id):
        cls.validate_input(input, checkout_id, user_id)
